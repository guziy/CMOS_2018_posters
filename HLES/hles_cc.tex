...
\adjustbox{valign=t, minipage=0.55\linewidth}{
  ...
  \midrule
  \makecell[tl]{\textbf{CRCM5\_HL}\\ 1979--2010}    & ERA-Interim (0.75$^\circ$) reanalysis-driven, all lakes are modelled by Hostetler (1D) lake model. \\[0.5cm]
  \makecell[tl]{\textbf{CRCM5\_NEMO}\\ 1979--2010}   & ERA-Interim (0.75$^\circ$) reanalysis-driven, the Great Lakes are modelled by NEMO (3D) ocean model, other lakes in the region are modelled by Hostetler (1D) lake model. \\[0.5cm]
  \makecell[tl]{\textbf{CRCM5\_NEMOc}\\ 1989--2010}  & Similar to \textbf{CRCM5\_NEMO}, but driven by CanESM2 outputs.\\[0.5cm]
  \makecell[tl]{\textbf{CRCM5\_NEMOf}\\ 2079--2100}  & Similar to \textbf{CRCM5\_NEMO}, but driven by CanESM2 outputs. \\
  \bottomrule
  \end{tabularx}
...
\adjustbox{valign=m, minipage=\linewidth}{
  ...
  \begin{itemize}
    ...
    \item CRCM5 time step: 5 min; NEMO time step: 30 min;
...
\begin{center}
    \includegraphics[width=\linewidth, trim={0 0 0 3.9cm}, clip]{figures/t2m_bias_crcm5_hl_vs_crcm5_nemo}
    \captionof{figure}{\label{figure:t2mvalidation}\color{Green} Seasonal biases in 2m air temperature ($^\circ$C) for CRCM5\_HL (first row)
                            with respect to \citet{hopkinson2011} and \citet{maurer2002} during the 1980--2010
                            period and differences between CRCM5\_NEMO and CRCM5\_HL-simulated 2m air
                            temperature (second row) during the 1980--2010 period.}
\end{center}

\begin{center}

  \begin{tikzpicture}
    \node (plot) {\includegraphics[width=\linewidth, trim={0 0 0 4cm}, clip]{figures/sst_validation_crcm5nemo_crcm5hl}};
    \node (winter)[font={\sffamily\footnotesize}, below right=5cm and 6cm of plot.north west, anchor=south west] {Winter};
    \node (spring)[font={\sffamily\footnotesize}, right=4cm of winter.east, anchor=west] {Spring};
    \node (summer)[font={\sffamily\footnotesize}, right=4cm of spring.east, anchor=west] {Summer};
    \node (fall)[font={\sffamily\footnotesize}, right=4cm of summer.east, anchor=west] {Fall};

  \end{tikzpicture}

    \captionof{figure}{\label{figure:sstvalidation}\color{Green}
    Seasonal biases of lake surface water temperature ($^\circ$C) in CRCM5\_HL (first row) and seasonal mean biases in the CRCM5\_NEMO (secod row) for the 2003--2010
    compared to lake surface water temperature ($^\circ$C) from NOAA OISST.}
\end{center}
...
\adjustbox{valign=t, minipage=0.48\linewidth}{
  \includegraphics[width=\linewidth, trim={0 0 0 3.9cm}, clip]{seasonal_biases_lake_ice_fraction_February-March_1980-2010}
  \captionof{figure}{\label{figure:ice2dvalidation}\color{Green} Lake ice concentration biases in
      CRCM5\_HL simulation (first row) compared to mean February and March ice
    fractions from the extended Great Lakes ice atlas, available from GLERL website,
    and ice fraction differences between CRCM5\_NEMO and CRCM5\_HL simulations
    (second row) for the 1980--2010 period.}
}
%
\hfill
%
\adjustbox{valign=t, minipage=0.48\linewidth}{
  \includegraphics[width=\linewidth, trim={0 0 0 3.9cm}, clip]{seasonal_biases_total_prec_Winter-Summer_1980-2010}
  \captionof{figure}{\label{figure:precipvalidation}\color{Green}
    Seasonal biases in CRCM5\_HL (first row) and differences between CRCM5\_NEMO and
    CRCM5\_HL-simulated total precipitation (second row) for the 1980-2010 period
    compared to Seasonal total precipitation (mm/day) from \citet{hopkinson2011} and
    \citet{maurer2002}.}
}
...
\adjustbox{valign=t, minipage=0.55\linewidth}{
\noindent
Simulated mean and inter-annual variability of HLES is improved in CRCM5\_NEMO, although it is still
underestimated (Fig.~\ref{figure:hlesinterannual}). The correlation of CRCM5\_NEMO (CRCM5\_HL)-based HLES
with the observation-based HLES is 0.72 (-0.10).
}
...
%
\adjustbox{valign=t, minipage=0.55\linewidth}{
      Distributions of HLES over months within the period of interest (Nov-Jan,
      Fig.~\ref{figure:hlesmonthly}) suggest that the fractions of HLES from
      observations and CRCM5\_NEMO are similar and increase from November to January.
      The distribution of HLES for CRCM5\_HL is quite different from the observed one,
      due to the negative lake water temperature biases in winter
      (Fig.~\ref{figure:sstvalidation}) and overestimated lake ice fractions
      (Fig.~\ref{figure:ice2dvalidation}), which lead to the shift of the majority of
      the HLES events to the onset period.
}

\subsection*{C.3 Linking HLES to large scale circulation patterns}

\noindent
\adjustbox{valign=t, minipage=0.40\linewidth}{
  \centering
...
%
\adjustbox{valign=t, minipage=0.55\linewidth}{
        Extreme high (low) HLES events are associated with the following two
...
\subsection*{C.4 Monthly distribution of mean HLES in current and future climates}


\noindent
\adjustbox{valign=t, minipage=\linewidth}{
  \begin{center}
        \includegraphics[width=\linewidth]{figures/hles_area_avg_cc}
  \end{center}
}

\adjustbox{valign=t, minipage=\linewidth}{
  \captionof{figure}{\color{Green} \label{figure:hles_monthly_distribution} (a) Monthly distributions of HLES in current (blue) and future (red) climates
    in \% of the total seasonal HLES. (b) Monthly distribution of the projected changes to HLES in \% of the current HLES amount for a given month.
    The projections are calculated from two 22-year CRCM5\_NEMO simulations driven by outputs from the second generation of Canadian Earth System Model.
    CRCM5\_NEMOc corresponds to the 1989--2010 period and CRCM5\_NEMOf corresponds to the 2079--2100 period.
  }
}
